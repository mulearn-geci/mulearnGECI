  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      try {
        ...
        setPinDuration(event.pinDuration || (event.isPinned ? 'always' : 'none'));
        setDisappearAfter(event.disappearAfter || 'never');
              ...
              <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700/60 pb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentEvent?.isPinned && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 dark:bg-amber-900/60 dark:text-amber-300 border border-amber-300 dark:border-amber-700">
                      <Pin className="w-3 h-3" />
                      {currentEvent.pinnedUntil
                        ? `Pinned until ${new Date(currentEvent.pinnedUntil).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}`
                        : 'Pinned (Always)'}
                    </span>
                  )}
                  {currentEvent?.expiresAt && (
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${
                      new Date(currentEvent.expiresAt) <= new Date()
                        ? 'bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-300 border border-red-300 dark:border-red-700'
                        : 'bg-blue-100 text-blue-800 dark:bg-blue-900/60 dark:text-blue-300 border border-blue-300 dark:border-blue-700'
                    }`}>
                      <EyeOff className="w-3 h-3" />
                      {new Date(currentEvent.expiresAt) <= new Date()
                        ? 'Expired / Hidden from public'
                        : `Disappears ${new Date(currentEvent.expiresAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}`}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
                    ...
                    {pinDuration === '1day' && 'Event will stay at the top for 24 hours from saving, then automatically unpin.'}
                    {pinDuration === '1week' && 'Event will stay at the top for 7 days from saving, then automatically unpin.'}
                    {pinDuration === '1month' && 'Event will stay at the top for 30 days from saving, then automatically unpin.'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
                    ...
                    {disappearAfter === '1day' && 'Event will disappear from the public website 24 hours after saving.'}
                    {disappearAfter === '1week' && 'Event will disappear from the public website 7 days after saving.'}
                    {disappearAfter === '1month' && 'Event will disappear from the public website 30 days after saving.'}
